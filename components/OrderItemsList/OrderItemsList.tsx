                ...
                cartPayload.items.map((item: any, itemIdx: number) => {
                    return <Flex key={`orderItem-${itemIdx}`} mx={4} my={2} flexGrow={1}>
                        ...
                        <Flex flexDir={"column"} alignItems={"flex-end"} justifyContent={'center'} pl={4}>
                            <Text fontSize={`xs`} color="var(--turbo-colors-light-text)">₹ {(item.price / 100).toFixed(2)} x {item.quantity}</Text>
                            <Text fontSize={`sm`} color="var(--turbo-colors-light-text)">₹ {(item.line_price/100).toFixed(2)}</Text>
                        </Flex>
                    </Flex>
                })
            }
